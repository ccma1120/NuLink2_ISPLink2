...
void MSC_Init(void)
{
    ...
    //g_TotalSectors = 128;
    g_TotalSectors = (8 * 1024 * 1024) / 512;
    g_u32MassBase = (unsigned int)&MassBlock;
    g_u32StorageBase = (unsigned int)&Storage_Block;
}
                ...
                case UFI_READ_10:
                {
                    ...
                    while (1)
                    {
                        if (total > BUFFER_PAGE_SIZE)
                        {
                            ...
                            SpiRead(g_u32LbaAddress, BUFFER_PAGE_SIZE, g_u32StorageBase);
                        ...
                        else
                        {
                            ...
                            SpiRead(g_u32LbaAddress, len, g_u32StorageBase);
                    ...
                    if (g_sCBW.bmCBWFlags == 0x00)      /* OUT */
                    {
                        ...
                        while (1)
                        {
                            if (total > BUFFER_PAGE_SIZE)
                            {
                                ...
                                SpiWrite(g_u32LbaAddress, BUFFER_PAGE_SIZE, g_u32StorageBase);
                                g_u32LbaAddress += len;
                                total -= len;
                            }
                            else
                            {
                                ...
                                SpiWrite(g_u32LbaAddress, len, g_u32StorageBase);
                                total = 0;
                                break;
                            }
                        }
